...
import com.safeword.store.LoginAttemptStore;
 ...
 * can lock you out of your family circle", which is a denial of service handed to the
...
public class LoginThrottle {
    ...
    public LoginThrottle(
            ...
            Clock clock,
            @Value("${safeword.security.login-throttle.max-failures-per-username:5}")
            int maxFailuresPerUsername,
            @Value("${safeword.security.login-throttle.max-failures-per-ip:20}")
            int maxFailuresPerIp,
            @Value("${safeword.security.login-throttle.window:PT15M}")
            Duration window,
            @Value("${safeword.security.login-throttle.max-registrations-per-ip:5}")
            int maxRegistrationsPerIp,
            @Value("${safeword.security.login-throttle.registration-window:PT1H}")
     ...
     */
    @Scheduled(initialDelayString = "${safeword.security.login-throttle.purge-initial-delay:PT15M}",
               fixedDelayString = "${safeword.security.login-throttle.purge-interval:PT15M}")
