...
public class LoginThrottle {
    ...
    public LoginThrottle(
            ...
            Clock clock,
            @Value("${refillradar.security.login-throttle.max-failures-per-username:5}")
            int maxFailuresPerUsername,
            @Value("${refillradar.security.login-throttle.max-failures-per-ip:20}")
            int maxFailuresPerIp,
            @Value("${refillradar.security.login-throttle.window:PT15M}")
            Duration window,
            @Value("${refillradar.security.login-throttle.max-registrations-per-ip:5}")
            int maxRegistrationsPerIp,
            @Value("${refillradar.security.login-throttle.registration-window:PT1H}")
     ...
     */
    @Scheduled(initialDelayString = "${refillradar.security.login-throttle.purge-initial-delay:PT15M}",
               fixedDelayString = "${refillradar.security.login-throttle.purge-interval:PT15M}")
